...
% Harris corner detecter function
...
rSize = size(R);
% The edge of the image tends to throw off the algorithm, so we are
% ignoring it
% Set edges to 0 (suppress). Based upon feature_width size which is passed in 
% Top edge
R(1: feature_width, :) = 0;
% Bottom edge
R(rSize(1) - feature_width: rSize(1), :) = 0;
% Left edge
R(:, 1:feature_width) = 0;
% Right edge
R(:, rSize(2) - feature_width:rSize(2)) = 0;

% Find max within R in a sliding 5 by 5 matrix (neighborhood)
maxValues = colfilt(R, [5 5], 'sliding', @max);
