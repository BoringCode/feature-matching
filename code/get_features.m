...
% Compute angles from gradient orientations
angles = angles + 180;
angles = angles .* pi / 180;

% Loop through possible features
% Looping is slow, but I'm not sure how else to do this
for i = 1:length(x)
    x_value = x(i);
    y_value = y(i);
    contribution_step = 1;
    descriptors = zeros(1, dimensions);
    % Grab angles (based on feature_width) from x and y coordinate
    % Why are the angles backwards?
    window_box = angles(y_value - feature_width / 2 + 1 : y_value + feature_width/2, x_value - feature_width / 2 + 1 : x_value + feature_width / 2);
    for Z = iterator
        for Y = iterator
            subwindow_angles = window_box(Z : Z + subwindow_step - 1, Y : Y + subwindow_step - 1);
            ...
            histogram = histcounts(orientations', bins);  
            ...
            histogram(histogram > clamp) = clamp;
            % Renormalize after clamping
            histogram = histogram / norm(histogram);
            % Set contributions to calculated histogram
            descriptors(contribution_step:contribution_step+(contribution_steps - 1)) = histogram;
            contribution_step = contribution_step + contribution_steps;
        end
    end
